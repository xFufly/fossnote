...
router.get('/', (req, res) => {
  ...
  session.createSession(session_id, 0, session_params, "", {key: key, iv: null}, {});

  const file_path = path.join(__dirname, '../../views/fossnote/fossnote.html');
  const html_content = fs.readFileSync(file_path, { encoding: 'utf-8' });
  var updated_html = html_content.replace("Start ()", "Start ({h:'" + session_params.h + "',d:" + session_params.d + ",sCrA:" + session_params.sCrA + ",sCoA:" + session_params.sCoA + ",poll:" + session_params.poll + "})");
